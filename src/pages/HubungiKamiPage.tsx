    ...
    <div className="bg-gray-50 text-gray-800 font-sans min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-6 pt-28 pb-16 md:pt-32 md:pb-24">
        <div className="text-center">
            ...
            Jika ada pertanyaan, saran, atau ingin berkolaborasi, silakan hubungi kami melalui kontak di bawah ini.
            </p>
        </div>
        
        <div className="flex flex-wrap justify-center gap-12">
            {contactList.map((contact) => (
                <div key={contact.id} className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-sm text-center transform hover:-translate-y-2 transition-transform duration-300 ease-in-out">
                    <img 
                        src={contact.imageUrl} 
                        alt={contact.name}
                        className="w-32 h-32 rounded-full mx-auto mb-6 border-4 border-blue-300 shadow-lg"
                    />
                    <h3 className="text-2xl font-bold text-blue-900 mb-2">{contact.name}</h3>
                    <a href={`mailto:${contact.email}`} className="block text-blue-600 hover:underline mb-3">
                        {contact.email}
                    </a>
                    <p className="text-gray-600">
                        Instagram: 
                        <a href={contact.instagram.url} target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-700 hover:underline ml-1">
